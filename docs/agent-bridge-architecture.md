...
`apps/example-app/test/commands.test.ts` does exactly this: it adds a todo and then compares `todos.list` from cache and from the server, which catches a broken `update` before the agent does, and it asserts that a failing save rolls the store back and fails the command.
