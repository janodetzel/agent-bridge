...
description: Run commands against the app in a simulator with `pnpm agent-bridge` to verify behavior, read state, or navigate. Use when checking whether a change actually works at runtime, when inspecting the Apollo cache or a Zustand store, when a bug reproduces only in the running app, or when a task says to verify in the simulator. Covers the exit codes, the cache-versus-network check, the connection rules, and when to reach for the ios-simulator or android-emulator skills instead.
---
...
```
pnpm agent-bridge todos.list --source cache
pnpm agent-bridge todos.list --source network
```
...
| Question                                                                   | Reach for                                                             |
| -------------------------------------------------------------------------- | --------------------------------------------------------------------- |
| Did the mutation land? What is in the cache or the store?                  | `pnpm agent-bridge <command>`                                         |
| Do cache and server agree after a write?                                   | `pnpm agent-bridge todos.list --source cache` then `--source network` |
