...
import { attachAppCommands } from "./attach";
 ...
 */
export function useAppCommands(registry: Registry, opts?: HandleOptions): void {
	const client = useDevToolsPluginClient(PLUGIN_NAME);
	const defaultTimeoutMs = opts?.defaultTimeoutMs;

	useEffect(() => {
		if (!client) return;
		return attachAppCommands(client, registry, { defaultTimeoutMs });
