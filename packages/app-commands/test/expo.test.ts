...
import { attachAppCommands, type BridgeClient } from "../src/expo/attach";
...
describe("attachAppCommands", () => {
	...
	it("stops answering after the returned function runs", async () => {
		const peer = fakeClient();
		const detach = attachAppCommands(peer.client, registry);
		detach();
